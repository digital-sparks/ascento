...
(() => {
  ...
  function initMultistepForm(form) {
    ...
    const requiredBusinessFieldNames = /* @__PURE__ */ new Set(["company", "country"]);
    ...
    function applyContactTypeVisibility(contactType) {
      ...
      companySlide.querySelectorAll("[data-form-title]").forEach((el) => {
        ...
        const shouldShow = category === "business" && showBusinessFields || category === "personal" && !showBusinessFields;
  ...
  function waitForResponseThenReveal(form, scope, successSlide, timeoutMs = 3e4) {
    let revealed = false;
